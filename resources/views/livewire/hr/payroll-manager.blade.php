                    ...
                    <td>
                        <div class="row-actions">
                            @if ($r['structure'])
                                ...
                                @endif
                                @if ($ps)
                                    <a class="btn btn--sm btn--secondary" href="{{ route('payslips.show', $ps->id) }}" target="_blank" rel="noopener" onclick="event.stopPropagation()">Payslip</a>
                                    @if ($ps->status === 'draft')<button class="btn btn--sm btn--secondary" wire:click.stop="finalize({{ $ps->id }})">Finalize</button>@endif
                                    @if ($ps->status === 'finalized')<button class="btn btn--sm btn--secondary" wire:click.stop="markPaid({{ $ps->id }})">Mark paid</button>@endif
                                @endif
                            @else
