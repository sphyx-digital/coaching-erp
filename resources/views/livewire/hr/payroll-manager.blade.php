                ...
                <tr wire:key="pr-{{ $s->id }}">
                    ...
                    <td>
                        <div class="row-actions">
                            <button class="btn btn--sm btn--secondary" wire:click="editStructure({{ $s->id }})">Salary</button>
                            @if ($r['structure'])
                                @if (! $ps || $ps->status === 'draft')
                                    <button class="btn btn--sm btn--primary" wire:click="generate({{ $s->id }})">{{ $ps ? 'Regenerate' : 'Generate' }}</button>
                                @endif
                                @if ($ps)
                                    <a class="btn btn--sm btn--secondary" href="{{ route('payslips.show', $ps->id) }}" target="_blank" rel="noopener">Payslip</a>
                                    @if ($ps->status === 'draft')<button class="btn btn--sm btn--secondary" wire:click="finalize({{ $ps->id }})">Finalize</button>@endif
                                    @if ($ps->status === 'finalized')<button class="btn btn--sm btn--secondary" wire:click="markPaid({{ $ps->id }})">Mark paid</button>@endif
                                @endif
                            @endif
                        </div>
                    </td>
