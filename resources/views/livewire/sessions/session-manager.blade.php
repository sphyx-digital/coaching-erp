                        ...
                        <tr wire:key="session-{{ $session->id }}" class="is-clickable" wire:click="view({{ $session->id }})" tabindex="0" wire:keydown.enter="view({{ $session->id }})">
                            ...
                            <td>
                                <div class="row-actions">
                                    @unless ($session->is_active)<x-btn size="sm" variant="secondary" wire:click.stop="markActive({{ $session->id }})">Set active</x-btn>@else<span class="field__hint">—</span>@endunless
                                </div>
                            </td>
                        </tr>
                    @endforeach
